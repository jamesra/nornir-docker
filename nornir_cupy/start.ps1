...
if (-not ($output.Contains($container_name))) {
    ...
    docker run --name $container_name -it -d --tmpfs /tmp --gpus all --net --net-alias $container_name --cap-add SYS_ADMIN --cap-add DAC_READ_SEARCH -v //storage2.connectomes.utah.edu/Data:/mnt/storage2 nornir
} 
